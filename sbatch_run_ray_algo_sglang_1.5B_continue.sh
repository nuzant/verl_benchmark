#!/bin/bash
#SBATCH --job-name=mzy-verl-algo-1.5B-n16-ctx27k-512x16
#SBATCH --nodes=16
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --mem-per-cpu=10G
#SBATCH --output=/storage/openpsi/users/meizhiyu.mzy/nips25/logs/%j-verl-algo-1.5B-n16-ctx27k-512x16.out
...
apptainer_image_path=/storage/openpsi/images/verl-sglang-math-74906b17.sif
# replace these information with your own
...
container_envs="--env PYTHONPATH=$workdir \
...
srun --mpi=pmi2 -K -l --chdir $workdir --overlap --nodes=1 --ntasks=1 -w "$head_node" \
    ...
    $container_envs \
    --bind /storage:/storage $apptainer_image_path \
        bash -c "ray status; bash ./nips25_algo/math_1.5B_16nodes_continue.sh"
