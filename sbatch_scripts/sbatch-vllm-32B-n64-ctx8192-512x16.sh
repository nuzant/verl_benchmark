#!/bin/bash
#SBATCH --job-name=mzy-verl-vllm-32B-n64-ctx8192-512x16
#SBATCH --nodes=64
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=120
#SBATCH --mem-per-cpu=10G
#SBATCH --exclude=slurmd-[20,40,62,63,78,88,94]
#SBATCH --output=/storage/openpsi/users/meizhiyu.mzy/nips25/logs/%j-verl-vllm-32B-n64-ctx8192-512x16.out
...
apptainer_image_path=/storage/openpsi/images/verl.sif
# replace these information with your own
...
srun --mpi=pmi2 -K -l --chdir $workdir --overlap --nodes=1 --ntasks=1 -w "$head_node" \
    ...
    $container_envs \
    --bind /storage:/storage $apptainer_image_path \
        bash -c "ray status; bash ./nips25/run-vllm-32B-n64-ctx8192-512x16.sh"
