#!/bin/bash
#SBATCH --job-name=mzy-verl-sglang-14B-n64-ctx32768-512x16
#SBATCH --nodes=64
...
#SBATCH --output=/storage/openpsi/users/meizhiyu.mzy/nips25/logs/%j-verl-sglang-14B-n64-ctx32768-512x16-2025051400.out
...
srun --mpi=pmi2 -K -l --chdir $workdir --overlap --nodes=1 --ntasks=1 -w "$head_node" \
    ...
    $container_envs \
    --bind /storage:/storage $apptainer_image_path \
        bash -c "ray status; bash ./nips25/run-sglang-14B-n64-ctx32768-512x16.sh"
