#!/bin/bash
#SBATCH --job-name=mzy-verl-vllm-{model_size}-n{nnodes}-ctx{max_response_length}-{batch_size}x{rollout_n}
#SBATCH --nodes={nnodes}
...
srun --mpi=pmi2 -K -l --chdir $workdir --overlap --nodes=1 --ntasks=1 -w "$head_node" \
    ...
    $container_envs \
    --bind /storage:/storage $apptainer_image_path \
        bash -c "ray status; bash ./nips25/run-vllm-{model_size}-n{nnodes}-ctx{max_response_length}-{batch_size}x{rollout_n}.sh"
