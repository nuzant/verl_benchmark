#!/bin/bash
#SBATCH --job-name=mzy-verl-algo-7B-n24-ctx27k-512x16
#SBATCH --nodes=24
...
#SBATCH --output=/storage/openpsi/users/meizhiyu.mzy/nips25/logs/%j-verl-algo-7B-n24-ctx27k-512x16.out
...
srun --mpi=pmi2 -K -l --chdir $workdir --overlap --nodes=1 --ntasks=1 -w "$head_node" \
    ...
    $container_envs \
    --bind /storage:/storage $apptainer_image_path \
        bash -c "ray status; bash ./nips25_algo/math_7B_24nodes.sh"
